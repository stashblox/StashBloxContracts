...
describe("ChargeableTransfer.sol", () => {
  ...
  describe("#_transfer", () => {
    ...
    it("it should split fees between feesRecipients", async () => {
      let ethBalance = (await STASHBLOX._users(DATA["token1"].feesRecipients[0])).ETHBalance
      ...
      ethBalance = (await STASHBLOX._users(DATA["token1"].feesRecipients[0])).ETHBalance
      assert.equal(ethBalance.toString(), storageGain1.toString(), "Incorrect balance increase");
      ethBalance = (await STASHBLOX._users(DATA["token1"].feesRecipients[1])).ETHBalance
